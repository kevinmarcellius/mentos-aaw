...
export const removeProductFromWishlistService = async (
    ...
    try {
        ...
        if (!wishlistDetail) {
            logger.error(`Wishlist detail not found for id: ${id}`);
        ...
        if (!wishlist) {
            logger.error(`Wishlist not found for id: ${wishlistDetail.wishlist_id}`);
            return new InternalServerErrorResponse('Wishlist not found').generate();
        }

        if (wishlist.user_id !== user.id) {
            logger.error(`User ${user.id} is not authorized to remove product from wishlist ${wishlist.id}`);
